...
class TestCHIPS(unittest.TestCase):
    ...
    def test_run_Hu(self):
        ...
        contours0, hierarchy = cv2.findContours(
            chips.astype(np.uint8), cv2.RETR_TREE, 
        ...
        annotations.append(
            Annotation(
                "2018-05-30 12:00", -0.08, 1., "left", "top", rotation=90,
            )
        )
        annotations.append(
            Annotation(
                "Scheme: CHIPS", 0.05, 1.05, "left", "center"
            )
        )
        ...
        contours1, hierarchy = cv2.findContours(
            np.flipud(charm).astype(np.uint8), cv2.RETR_TREE, 
            cv2.CHAIN_APPROX_SIMPLE
        )
        img = np.zeros((resolution, resolution))
        ...
        annotations.append(
            Annotation(
                "Scheme: CHARM", 0.05, 1.05, "left", "center"
            )
        )
        annotations.append(
            Annotation(
                "Hu: %.2f"%sim, 0.99, 1.05, "right", "center"
            )
        )
        ...
        contours2, hierarchy = cv2.findContours(
            np.flipud(catch).astype(np.uint8), cv2.RETR_TREE, 
            cv2.CHAIN_APPROX_SIMPLE
        )
        img = np.zeros((resolution, resolution))
        ...
        annotations.append(
            Annotation(
                "Scheme: CATCH", 0.05, 1.05, "left", "center"
            )
        )
        annotations.append(
            Annotation(
