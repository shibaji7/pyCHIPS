...
class CleanFl(object):
    ...
    def create_coronal_hole_candidates(
        ...
        def scale_limits(data, lims, max=255):
            ...
            """
            return np.array(
                255 * (data - lims[0]) / (lims[1] - lims[0]), dtype=np.float32
            )
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            bmmix[
                scaled_log_data171 / scaled_log_data211
                >= (
                    np.mean(data171_rescaled)
                    * self.params.bmmix_value
                    / np.mean(data211_rescaled)
                )
            ] = 1
            bmhot[
                scaled_log_data211 + scaled_log_data193
                < (
                    self.params.bmhot_value
                    * (np.mean(data193_rescaled) + np.mean(data211_rescaled))
                )
            ] = 1
            bmcool[
                scaled_log_data171 / scaled_log_data193
                >= (
                    np.mean(data171_rescaled)
                    * self.params.bmcool_value
                    / np.mean(data193_rescaled)
                )
            ] = 1
    ...
    def produce_summary_plots(
        ...
        ax = draw_panel(
            ip.__axis__(axis_off=False), self.bmcool, r"193 $\AA$ / 171 $\AA$"
        )
