...
"""to_remote.py: Fetch data from the remote locations"""
...
def get_pubfile():
    with open("data/config/pub.json", "r") as f:
        obj = json.loads("".join(f.readlines()))
        pubfile = obj["pubfile"]
    return pubfile

def get_session(filename="data/config/passcode.json", key_filename=None, isclose=False):
